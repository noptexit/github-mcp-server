...
func BenchmarkWithScopeChallengeDynamicPolicy(b *testing.B) {
	scopes.SetGlobalToolScopeMap(scopes.ToolScopeMap{
		"write_file": scopes.DynamicChallenge(
			[]scopes.Scope{scopes.Repo, scopes.Workflow},
			func([]string) bool { return true },
			func(arguments map[string]any, activeScopes []string) []string {
				if path, _ := arguments["path"].(string); strings.HasPrefix(path, ".github/workflows/") {
	...
	b.Cleanup(func() { scopes.SetGlobalToolScopeMap(nil) })

	for _, payload := range []struct {
		name string
		size int
	}{
		{name: "small_arguments", size: 256},
		{name: "large_arguments", size: 32 * 1024},
	} {
		body := `{"jsonrpc":"2.0","method":"tools/call","params":{"name":"write_file","arguments":{"path":"README.md","nested":[{"payload":"` +
			strings.Repeat("x", payload.size) + `"},[1,2,3]]}}}`
		next := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			var downstreamRequest mcpJSONRPCRequest
			_ = json.NewDecoder(r.Body).Decode(&downstreamRequest)
			var arguments map[string]any
			_ = json.Unmarshal(downstreamRequest.Params.Arguments, &arguments)
			w.WriteHeader(http.StatusNoContent)
		})
		handler := WithMCPParse()(WithScopeChallenge(&oauth.Config{}, &mockScopeFetcher{})(next))
		...
		b.Run(payload.name, func(b *testing.B) {
			for _, benchmark := range []struct {
				...
				{name: "maximum_scopes_fast_path", activeScopes: []string{"repo", "workflow"}},
				{name: "call_specific_decode", activeScopes: []string{"repo"}},
			} {
				b.Run(benchmark.name, func(b *testing.B) {
					...
					for b.Loop() {
						request := httptest.NewRequest(http.MethodPost, "/mcp", strings.NewReader(body))
						request = request.WithContext(scopeChallengeContextWithScopes(request.Context(), benchmark.activeScopes))
